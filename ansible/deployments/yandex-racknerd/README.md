...
Оба lane работают. Для новых клиентов по умолчанию удобнее **lane 2** (`split2`) — отдельный exit и стабильный UDP-путь. Lane 1 (`split`) снова рабочий после смены transit listen-порта на `49251` (см. `host_vars/entry_split_01.yml`).

| Transit | Listen на entry | Peer на exit | Порт exit |
|---------|-----------------|--------------|-----------|
| lane 1 `wg-transit` | `49251` | `exit_01:51821` | `51821` |
| lane 2 `wg-transit-2` | `49249` | `exit_02:51821` | `51821` |

## DNS на entry

| Lane | DNS для клиентов | Сервис | Upstream (reference) |
|------|------------------|--------|----------------------|
| 1 | `10.60.0.1` | `dnsmasq` на `wg-client` | `1.1.1.1`, `8.8.8.8` |
| 2 | `10.61.0.1` | `dnsmasq` на `wg-client-2` | `1.1.1.1`, `8.8.8.8` |

Проверка: `dig @10.61.0.1 yandex.ru` с entry.
